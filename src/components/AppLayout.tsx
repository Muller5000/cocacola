...
export default function AppLayout({ 
  ...
  return (
    <div className="flex min-h-screen bg-[#0A0A0A] text-white font-sans antialiased overflow-x-hidden">
      {/* Sidebar */}
      <aside className="w-[280px] flex flex-col fixed left-0 top-0 h-screen z-30 bg-[#141416] border-r border-[#232326]">
        <div className="flex items-center gap-3 px-8 py-10 mb-4">
      ...
      <main className="flex-1 flex flex-col min-h-screen ml-[280px]">
        {/* Topbar */}
        <header className="h-24 flex items-center justify-between px-10 sticky top-0 z-20 backdrop-blur-md bg-[#0A0A0A]/80 border-b border-[#1C1C1E]">
          <div className="flex flex-col">
            <p className="text-[10px] font-bold text-[#8E8E93] uppercase tracking-widest">Command Center / <span className="text-white">Live Intelligence</span></p>
            <h1 className="text-2xl font-bold text-white tracking-tight mt-1">Supply Chain Dashboard</h1>
          </div>

          <div className="flex items-center gap-4 p-1.5 bg-[#1C1C1E] rounded-2xl border border-[#232326]">
            ...
            <button className="p-2 text-[#48484A] hover:text-[#F40009] transition-colors relative">
              <Bell className="w-5 h-5" />
              <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-[#F40009] rounded-full border-2 border-[#1C1C1E]"></span>
            </button>
            <div className="w-px h-6 bg-[#232326]"></div>
        ...
        <div className="px-10 pb-10 pt-10 flex-1">
          {children}
        </div>
      </main>
    </div>
